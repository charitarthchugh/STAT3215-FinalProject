...
```{r}
summary(pT)
testTransform(pT, lambda = c(0, 0, 0, 0, -2, 0, -3, 0)) # use appropriate parameter estimates
testTransform(pT, lambda = c(0.1, -0.4, -0.3, 0.2, -1.6, -0.1, -29, 0.1))
```
...
```{r}
m5 <- lm( TRANSFORMED MODEL, data = df_train)
```
...
ncvTest(m5)
# scatterplot matrix: normality, linearity
pairs.panels(df_transformed_vars[a:b], ellipses=FALSE, hist.col = "cyan",
method = "pearson", density = TRUE)
# residual plots: constant variance, linearity
residualPlots(m5, type="rstudent")
...
The metrics output from `regsubset()` will be examined in section 4.4 discussing model selection.
...
Our enhancement attempts to address these violations by {...}

We conclude that we can create a better multiple linear regression model based on additional metrics considered in this enhancement that were not included in the original paper. Our goal was to identify a model {...}
