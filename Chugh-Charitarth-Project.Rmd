...
qqnorm(ti); qqline(ti)
m<-mean(ti); std<-sd(ti)
hist(ti, density=20, breaks=20, prob=TRUE, xlab="Studentized residuals", # including histogram overlay
...
```{r pairs-panels-transformed eval=FALSE}
# transformed scatterplot matrix: normality, linearity
pairs.panels(df_transformed_vars_train[,10:17], ellipses=FALSE, hist.col = "cyan", 
             method = "pearson", density = TRUE)
```

```{r residual-plots-transformed eval=FALSE}
...
### 4.3 Variable Selection for Discovery
...
df_transformed_vars_train_test <- transform(df_test_shifted,
  logDepDelay=log(DepDelay), logCarrierDelay=log(CarrierDelay),
  logTaxiIn=log(TaxiIn), logTaxiOut=log(TaxiOut),
  logDistance=log(Distance), inv2WeatherDelay=I(WeatherDelay^(-2)), 
  logNASDelay=log(NASDelay), SecurityDelay1=SecurityDelay,
...
x.test<- as.matrix(df_transformed_vars_train_test[,10:18]) 
y.test<- df_transformed_vars_train_test$logDepDelay
x.train <- as.matrix(df_transformed_vars_train[,10:18])
y.train <- df_transformed_vars_train$logDepDelay
n.test <- dim(df_transformed_vars_train_test)[1]
...
### 4.4 Model Selection for Prediction
...
MAE (mean absolute prediction error) is the average absolute prediction error, and is less sensitive to outliers. MAE is computed as: $$\frac{\sum_{i=1}^{n_H}|y_{H_i}-\hat{y}^*_{H_i}|}{n_H}$$ 
Our goal is to find a model that minimizes both MSE and MAE, so that the model is more robust in predicting capability.

VEcv is the amount of variance explained based on cross-validation, computed as: $$1-\frac{\sum_{i=1}^{n_H}(y_{H_i}-\hat{y}^*_{H_i})^2}{\sum_{i=1}^{n_H}(y_{H_i}-\bar{y}_{H_i})^2}$$
Where $\bar{y}_{H}$ is sample mean of test responses $y_{H_i}$ of all test observations. 

#### 4.4.1 Subset Selection Evaluation

```{r modelEvaluations}
 
m7.predicted <- predict(m7, newdata=as.data.frame(x.test))
sse <- sum((y.test - m7.predicted)^2)
m7.out <- c(mse=sse/n.test, mae= mean(abs(y.test - m7.predicted)), VEcv= (1-sse/syy.test)*100) 
m7.out
m8.predicted <- predict(m8, newdata=as.data.frame(x.test))
sse <- sum((y.test - m8.predicted)^2)
m8.out <- c(mse=sse/n.test, mae= mean(abs(y.test - m8.predicted)), VEcv= (1-sse/syy.test)*100) 
m8.out

```
#### 4.4.2 LASSO Evaluation

#### 4.4.3 Adaptive LASSO Evaluation

#### 4.4.4 Elastic Net Evaluation

### 4.4.5 SCAD Evaluation

### 4.4.6 Control Evaluation
```{r control-eval}
m1 <- lm(exp(logDepDelay) ~ exp(logCarrierDelay) + exp(logTaxiIn) + exp(logTaxiOut) + 
    exp(logDistance) + I(inv2WeatherDelay^(-1/2)) + exp(logNASDelay) + SecurityDelay1 + 
    exp(logLateAircraftDelay), data = df_transformed_vars)
m1.predicted <- predict(m1, newdata=as.data.frame(x.test))
sse <- sum((y.test - m1.predicted)^2)
m1.out <- c(mse=sse/n.test, mae= mean(abs(y.test - m7.predicted)), VEcv= (1-sse/syy.test)*100) 
m1.out
